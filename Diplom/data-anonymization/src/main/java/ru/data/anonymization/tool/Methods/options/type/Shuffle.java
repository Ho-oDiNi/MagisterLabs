...
public class Shuffle implements MaskItem {
    ...
    public void start(DatabaseConnectionService controllerDB) throws Exception {
        String columnsRow = convertListToString(namesColumn, ",");
        String newNameTable = "mixing_instructions_"+nameTable;
        List<Integer> oldPosition = new ArrayList<>();
        List<Integer> newPosition = new ArrayList<>();

        controllerDB.execute("CREATE TABLE " + newNameTable + " (id SERIAL PRIMARY KEY,old_position INT,new_position INT);");
        controllerDB.execute("ALTER TABLE " + nameTable + " ADD COLUMN masking_id_for_shuffle INT GENERATED BY DEFAULT AS IDENTITY UNIQUE;");
        ResultSet resultSet = controllerDB.executeQuery("select count(*) from " + nameTable + ";");
        resultSet.next();
        int sizeTable = resultSet.getInt(1);
        resultSet = controllerDB.executeQuery("select " + columnsRow + ",masking_id_for_shuffle from " + nameTable + ";");
        ...
        controllerDB.execute("ALTER TABLE " + nameTable + " DROP COLUMN masking_id_for_shuffle;");

        for (int i = 0; i < oldPosition.size(); i++) {
            controllerDB.execute("INSERT INTO "+newNameTable+" (old_position,new_position) values("+oldPosition.get(i)+", "+newPosition.get(i)+");");
        }
    }
}
