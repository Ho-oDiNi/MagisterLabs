...
def createTables(dbFileName):
    ...
    cur.execute("CREATE TABLE db_fltWordList("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "word TEXT)"
    )
    
    cur.execute("CREATE TABLE db_unFltWordList("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "word TEXT)"
    )

    cur.execute("CREATE TABLE db_fltWordLocation("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "fk_word_id INTEGER, "
                "fk_url_id INTEGER, "
                "location INTEGER)"
    ...
    cur.execute("CREATE TABLE db_unFltWordLocation("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "fk_word_id INTEGER, "
                "fk_url_id INTEGER, "
                "location INTEGER)"
    ...
    cur.execute("CREATE TABLE db_fltLinkWord("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "fk_word_id INTEGER, "
                "fk_link_id INTEGER)"
    )

    cur.execute("CREATE TABLE db_unFltLinkWord("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                "fk_word_id INTEGER, "
                "fk_link_id INTEGER)"
    )
...
def deleteTables(dbFileName):
    ...
    cur.execute("DROP TABLE db_unFltWordList")
    cur.execute("DROP TABLE db_fltWordList")
    cur.execute("DROP TABLE db_fltWordLocation")
    cur.execute("DROP TABLE db_unFltWordLocation")
    cur.execute("DROP TABLE db_fltLinkWord")
    cur.execute("DROP TABLE db_unFltLinkWord")
...
def createCsv(CSVfilename):
    with open(CSVfilename, 'a', newline='') as csvfile:
        ...
        spamwriter.writerow(["UrlListLen", "linkBetweenURLLen", "fltWorlListLen", "unFltWorlListLen"])
...
class Crawler:
    ...
    def addIndex(self, soup, url):
        ...
        fltTextList, unFltTextList = self.getTextOnly(soup)
        
        url_id = self.getEntryId('db_urlList', 'url', url)  #   Получаем идентификатор URL
        self.addTextToDB(url_id, fltTextList, unFltTextList)
    ...
    def addLinkRef(self, urlFrom:str, urlTo:str, linkText:str):
        ...
        for word in filteredTextLink:
            word_id = self.getEntryId('db_fltWordList', 'word', word)
            link_word_id = self.getEntryId('db_fltLinkWord', 'fk_link_id', link_id, 'fk_word_id', word_id)

        for word in linkText:
            word_id = self.getEntryId('db_unFltWordList', 'word', word.lower())
            link_word_id = self.getEntryId('db_unFltLinkWord', 'fk_link_id', link_id, 'fk_word_id', word_id)
    ...
    def addTextToDB(self, url_id, fltTextList, unFltTextList):
        ...
        if fltTextList != None:
            ...
            for word in fltTextList:
                location = location + 1
                word_id = self.getEntryId('db_fltWordList', 'word', word)
                word_location_id = self.getEntryId('db_fltWordLocation', 'fk_url_id', url_id, 'fk_word_id', word_id, 'location', location) 

        if unFltTextList != None:
            location = 0
            for word in unFltTextList:
                location = location + 1
                word_id = self.getEntryId('db_unFltWordList', 'word', word)
                word_location_id = self.getEntryId('db_unFltwordLocation', 'fk_url_id', url_id, 'fk_word_id', word_id, 'location', location) 
    ...
    def textFilterToUnFilter(self, text):

        clearText = self.separateWords(text)
    ...
    def writeCsv(self, UrlListLen, linkBetweenURLLen, fltWorlListLen, unFltWorlListLen):
    ...
    def check_db(self):
        ...
        requestGetFLTWorlList = f"SELECT * FROM db_fltWordList"
        requestGetUnFLTWorlList = f"SELECT * FROM db_fltWordList"
        ...
        fltWorlListLen = len(cur.execute(f"{requestGetFLTWorlList}").fetchall())
        unFltWorlListLen = len(cur.execute(f"{requestGetUnFLTWorlList}").fetchall())
        
        self.writeCsv(UrlListLen, linkBetweenURLLen, fltWorlListLen, unFltWorlListLen)
